...
class SearchCommand(object):
    ...
    def input_header(self):
        ...
        """
        warn(
            "SearchCommand.input_header is deprecated and will be removed in a future"
            " release. Please use SearchCommand.metadata instead.",
            DeprecationWarning,
            2,
        )
    ...
    def _process_protocol_v1(self, argv, ifile, ofile):
        ...
        try:
            if argv[1] == "__GETINFO__":
                ...
                message = (
                    "Command {0} appears to be statically configured for search command"
                    " protocol version 1 and static configuration is unsupported by"
                    " splunklib.searchcommands. Please ensure that"
    ...
    def _execute_chunk_v2(self, process, chunk):
        ...
        if len(body) <= 0 and not self._allow_empty_input:
            raise ValueError(
                "No records found to process. Set allow_empty_input=True in dispatch"
                " function to move forward with empty records."
            )
