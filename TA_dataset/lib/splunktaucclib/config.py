...
class Config:
    ...
    def update_items(
        ...
        ), "Unexpected endpoint id in given schema - {ep_id}".format(ep_id=endpoint_id)
        ...
        for item_name in item_names:
            ...
            if resp is None or resp.status_code not in (200, 201):
                msg = (
                    f'Fail to update item "{item_name}" in endpoint "{endpoint_id}" -'
                    f" {code_to_msg(resp)}"
                )
    ...
    def make_uri(self, endpoint_id, item_name=None):
        ...
        ep_uri = (
            ...
            else "{splunkd_uri}/servicesNS/{user}/{app}/{endpoint_full}".format(
