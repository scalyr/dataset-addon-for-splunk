...
You need two Python environments. Python 3.8 for development and package building and Python 3.11 for `pre-commit` and linting tools.

### Python 3.8

* Install Python 3.8 `brew install python@3.8`
* Create venv with Python3.8: `python3.8 -m venv venv-3.8`
* Activate it: `source venv-3.8/bin/activate`
...
* Install `pre-commit`:
  ...
  pre-commit install
  ```
  * If this fails, you can create following file - `.git/hooks/pre-commit`:
    ```bash
    #!/usr/bin/env bash
